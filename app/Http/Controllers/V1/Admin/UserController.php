...
class UserController extends Controller
{
    ...
    public function dumpCSV(Request $request)
    {
        ...
        foreach($res as $user) {
            $expireDate = $user['expired_at'] === NULL ? 'Vĩnh Viễn' : date('Y-m-d H:i:s', $user['expired_at']);
            $balance = $user['balance'] / 100;
            $commissionBalance = $user['commission_balance'] / 100;
            $transferEnable = $user['transfer_enable'] ? $user['transfer_enable'] / 1073741824 : 0;
            $deviceLimit = $user['devce_limit'] ? $user['devce_limit'] : NULL;
            $notUseFlow = (($user['transfer_enable'] - ($user['u'] + $user['d'])) / 1073741824) ?? 0;
            $planName = $user['plan_name'] ?? 'Không đăng ký';
    ...
    public function ban(Request $request)
    {
        ...
        try {
            ...
            abort(500, 'Quá trình thực hiện bị lỗi');
        }
    ...
    public function delUser(Request $request)
    {
        ...
        try {
            ...
            abort(500, 'Không thể xóa đơn đặt hàng của người dùng');
        }

        try {
            $inviteUser = User::where('invite_user_id', $request->input('id'))->update(['invite_user_id' => null]);
        } catch (\Exception $e) {
            abort(500, 'Không xóa được người mời người dùng');
        }
