...
class AuthController extends Controller
{
    ...
    public function register(AuthRegister $request)
    {
        ...
        $domain = $parsedUrl['host'] ?? null; // Lấy phần host từ URL

        // Kiểm tra domain trong database
        $staffUser = null;
        if ($domain) {
            $staffUser = User::where('staff_url', $domain)->first();
        }

        // Nếu có staffUser, cập nhật invite_user_id dựa trên staffUser id
        if ($staffUser) {
            $inviteUserId = $staffUser->id;
        } else {
            // Xử lý invite_code như bình thường
            if ($request->input('invite_code')) {
                ...
                if (!$inviteCode) {
                    ...
                    $inviteUserId = $inviteCode->user_id ? $inviteCode->user_id : null;
    ...
    // public function login(AuthLogin $request)
    // {
    //     $identifier = $request->input('email'); // This can be either email or username
    //     $password = $request->input('password');
