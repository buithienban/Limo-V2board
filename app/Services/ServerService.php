...
class ServerService
{
    ...
    public function getAvailableServers(User $user)
    {
        ...
        $this->mergeData($servers); // Sử dụng phương thức mergeData để thêm thông tin online
    ...
    private function addOnlineUsersInfo(&$servers)
    {
        foreach ($servers as $key => $server) {
            
            $onlineUsersCount = Cache::get(CacheKey::get("SERVER_{$server['type']}_ONLINE_USER", $server['id']), 0);
            
            $servers[$key]['online_users'] = $onlineUsersCount;
        }
    }
    ...
    public function getServersByStaffPlanIds($staffPlanIds) {
        
        $groupIds = Plan::whereIn('id', $staffPlanIds)->pluck('group_id');
        
        
        $servers = Server::whereIn('group_id', $groupIds)->get();
