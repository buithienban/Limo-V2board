...
class UserService
{
    ...
    private function calcResetDayByDaily(int $expiredAt): string
    {
        // $expiryHour = date('H', $expiredAt);

    
        // $currentTime = time();

        
        // $nextExpiryTime = strtotime('+1 day', strtotime(date('Y-m-d') . " $expiryHour:00:00"));

        
        // $minutesDiff = round(($nextExpiryTime - $currentTime) / 60);

        // return $minutesDiff;
        $time = date('H i', $expiredAt);
        return str_replace(' ', ' giờ ', $time) . ' phút';
    }
        ...
        switch (true) {
            case ($user->plan->reset_traffic_method === NULL): {
                ...
                switch ((int)$resetTrafficMethod) {
                    ...
                    case 5:
                        return $this->calcResetDayByDaily($user->expired_at);
                }
                break;
            }
    ...
    public function getStaffPlanIds($user) {
        
        $staffPlanIds = explode(',', $user->staff_plan);
        
        
        $staffPlanIds = array_map('intval', $staffPlanIds);
